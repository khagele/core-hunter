...
function row(rec, nowMs, onSelect) {
...
export function createTargetList(listEl, { onSelect, pinnedEl } = {}) {
  ...
  function render(rows, ignore, nowMs) {
    lastRows = rows
    lastIgnore = ignore

    if (pinnedEl) {
      const pinned = topSenders(rows, { ignore, count: PINNED_COUNT, nowMs })
      const pinnedSig = pinned.map((r) => (r.sender_label || r.sender_id || '') + r.rssi + r.rx_at).join('|')
      if (pinnedSig !== _lastPinnedSig) {
        _lastPinnedSig = pinnedSig
        pinnedEl.replaceChildren(...pinned.map((rec) => row(rec, nowMs, onSelect)))
      }
    }

    const items = senderList(rows, { ignore, limit: visible })
    const sig = items.map((r) => (r.sender_label || r.sender_id || '') + r.rssi + r.rx_at).join('|') + '#' + visible
    if (sig === _lastSig) return
    _lastSig = sig
    listEl.replaceChildren(...items.map((rec) => row(rec, nowMs, onSelect)))
  }
  ...
  listEl.addEventListener('scroll', () => {
    ...
    visible += PAGE_SIZE
    _lastSig = null
    render(lastRows, lastIgnore, Date.now())
  })
